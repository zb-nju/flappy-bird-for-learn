...
class Bird(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, images, brain = BirdBrain()):
    ...
    def flap(self):
        if self.y > -2 * Bird.HEIGHT: # avoid fly too high
    ...
    def update(self, delta_frames=1):
        ...
        if self.vel_y < Bird.MAX_VEL_Y and not self.flapped:
            self.vel_y += self.acc_y
        if self.flapped:
            self.flapped = False
            self.rot = 45
            
        self.y += min(self.vel_y,WIN_HEIGHT-self.y-Bird.HEIGHT)
    ...
    def image(self):
        ...
        visible_rot = min(self.rot,Bird.ROT_THR)
        return pygame.transform.rotate(img, visible_rot)
    @property
    def mask(self):
...
class GameView():
    ...
    def render(self,pipes,birds,score):
...
class FlappyBirdGame():
    ...
    def reset(self,birdNum = -1,birdBrains = []):
        self.pipes = deque()       
        self.score = 0
    ...
    def run(self):
        ...
        while not done:
            ...
            # render 
            view.render(self.pipes,self.birds,score) 
...
if __name__ == '__main__':
    ...
    g = FlappyBirdGame(60,1,[BirdBrain()])
    g.run()
